<?php
 ...
 if($error == 0)
 {
  $crear = mysqli_query($conexion,"insert into meta_sell_out (distribuidor,anio,linea) values ('$id_distribuidor','$id_anio','$id_linea')");
  ...
  while ($resultado_consulta_q = mysqli_fetch_array($consulta_q))
  {
   if($resultado_consulta_q['nombre'] == 'Q1')
   {
	...
	$crear = mysqli_query($conexion,"insert into meta_q_sell_out (meta_sell_out,q,meta,comision) values ('$id_crear','$id_q','$q1','$cq1')");
   }
   if($resultado_consulta_q['nombre'] == 'Q2')
   {
    $id_q = $resultado_consulta_q['id'];
	$crear = mysqli_query($conexion,"insert into meta_q_sell_out (meta_sell_out,q,meta,comision) values ('$id_crear','$id_q','$q2','$cq2')");
   }
   if($resultado_consulta_q['nombre'] == 'Q3')
   {
    $id_q = $resultado_consulta_q['id'];
	$crear = mysqli_query($conexion,"insert into meta_q_sell_out (meta_sell_out,q,meta,comision) values ('$id_crear','$id_q','$q3','$cq3')");
   }
   if($resultado_consulta_q['nombre'] == 'Q4')
   {
    $id_q = $resultado_consulta_q['id'];
	$crear = mysqli_query($conexion,"insert into meta_q_sell_out (meta_sell_out,q,meta,comision) values ('$id_crear','$id_q','$q4','$cq4')");
   }
  }
